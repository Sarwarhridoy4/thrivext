...
export const metadata = {
    title: 'Docs',  // Title displayed in browser tab
        ...
        <div className="space-y-4 text-center items-center flex flex-col">
          ...
          {courses.map((course) => (
            <Link href={course.href} key={course.id} className="rounded-xl flex items-center justify-center bg-[#0D0E1F]">
              <Image alt="next" width={80} height={80} className="w-20 p-3" src={next} />
              <div className={`${course.color} text-xl font-bold bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 text-transparent bg-clip-text hover:underline p-3.5 ps-0 rounded-lg hover:bg-white transition-all duration-300 inline-block`}>
                {course.des}
              </div>
            </Link>
          ))}
